...
/* open_6_Shells
 *
 *  DESCRIPTION: this function checks if 6 shells can be opened
 *  INPUT: fname
 *  OUTPUT: PASS/FAIL
 *  SIDE EFFECTS: 
 */
int test_print_RANDOM() {
	printf("execute command : garbage");
	execute((uint8_t *)"garbage");

	printf("exeucte command '   garbage'");
	execute((uint8_t *)"   garbage");
...
int32_t read_random() {
	printf("read NULL");
	if (read (-1, NULL, -1) == FAIL){
		return PASS;
	}
	return FAIL;
}

int32_t write_random() {
	printf("write NULL");
	if (read (-1, NULL, -1) == FAIL){
		return PASS;
	}
...
void launch_tests(){
	...
	/*------------------------------------Test for Checkpoint 3.3---------------------------------------------*/
	printf("====================TEST FOR RANDOM BEGINS ============================\n"); 
	TEST_OUTPUT("test_print", test_print_RANDOM());

	printf("====================TEST FOR OPEN BEGINS ============================\n"); 
	TEST_OUTPUT("test_print", open_random());

	printf("====================TEST FOR CLOSE BEGINS ============================\n"); 
	TEST_OUTPUT("test_print", close_random());

	printf("====================TEST FOR READ BEGINS ============================\n"); 
	TEST_OUTPUT("test_print", read_random());

	printf("====================TEST FOR WRITE BEGINS ============================\n"); 
	TEST_OUTPUT("test_print", write_random());
}
